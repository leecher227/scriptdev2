...
SDAuthor: Tassadar + PSZ + some corrections from MaxXx2021
...
struct MANGOS_DLL_DECL boss_malygosAI : public ScriptedAI
{
    ...
    std::list<uint64> m_lHoverDiskGUIDList;
    ...
    void Reset()
    {
        ...
        if (!m_lSparkPortalGUIDList.empty())
            for (std::list<uint64>::iterator itr = m_lSparkPortalGUIDList.begin(); itr != m_lSparkPortalGUIDList.end(); ++itr)
                if (Creature* pSparkPortal = (Creature*)Unit::GetUnit(*m_creature, *itr))
    ...
    void DespawnCreatures(uint32 uiEntry)
    {
        ...
        if (uiEntry == NPC_HOVER_DISK)
        {
            if (m_lHoverDiskGUIDList.empty())
                return;
    ...
    void MovementInform(uint32 uiType, uint32 uiPointId)
    {
        ...
        {
            if (Creature* pTargetSparkPortal = (Creature*)Unit::GetUnit(*m_creature, m_uiTargetSparkPortalGUID))
        ...
        else if (uiPointId == POINT_ID_PHASE_2_WP)
        {
            if (m_uiSubPhase == SUBPHASE_TALK)
            {
                ...
                {
                    if (Creature* pLord = m_creature->SummonCreature(NPC_NEXUS_LORD, urand(PLATFORM_MIN_X, PLATFORM_MAX_X), urand(PLATFORM_MIN_Y, PLATFORM_MAX_Y), FLOOR_Z+10.0f+urand(0, 15), 0, TEMPSUMMON_CORPSE_DESPAWN, 0))
						pLord->SetInCombatWithZone();
                }
				for (uint8 i = 0; i < (m_bIsRegularMode ? SCION_OF_ETERNITY_COUNT : SCION_OF_ETERNITY_COUNT_H); ++i)
				{
					if (Creature* pScion = m_creature->SummonCreature(NPC_SCION_OF_ETERNITY, urand(PLATFORM_MIN_X, PLATFORM_MAX_X), urand(PLATFORM_MIN_Y, PLATFORM_MAX_Y), FLOOR_Z+10.0f+urand(0, 15), 0, TEMPSUMMON_CORPSE_DESPAWN, 0))
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_uiPhase == PHASE_INTRO && m_creature->GetMotionMaster()->GetCurrentMovementGeneratorType() != POINT_MOTION_TYPE)
        {
            if (m_bReadyForWPMove)
            {
                if (m_uiTimer <= uiDiff)
                {
                    ...
                    m_creature->GetMotionMaster()->Clear();
                    m_creature->GetMotionMaster()->MovePoint(POINT_ID_WAYPOINT, WPs[m_uiLastWP].x, WPs[m_uiLastWP].y, AIR_Z);
        ...
        if (m_uiPhase == PHASE_FLOOR)
        {
            if (m_uiSubPhase == SUBPHASE_VORTEX)
            {
                if (m_uiVortexPhase > 0 && m_uiTimer <= uiDiff)
                {
                    ...
                    else if (m_uiVortexPhase > 3 && m_uiVortexPhase < MAX_VORTEX+3)
                    {
                        ...
                        if (pMap)
                        {
                            if (Creature *pVortex = m_creature->SummonCreature(NPC_VORTEX, VortexLoc[m_uiVortexPhase-4].x, VortexLoc[m_uiVortexPhase-4].y, FLOOR_Z+20.0f, 0, TEMPSUMMON_TIMED_DESPAWN, 3000))
                            {
                                Map::PlayerList const &lPlayers = pMap->GetPlayers();
                                for (Map::PlayerList::const_iterator itr = lPlayers.begin(); itr != lPlayers.end(); ++itr)
                                {
            ...
            if (m_uiPowerSparkTimer <= uiDiff)
            {
                ...
                if (Creature* pTargetSparkPortal = (Creature*)Unit::GetUnit(*m_creature, *pTargetSparkPortalGUID))
        ...
        else if (m_uiPhase == PHASE_ADDS)
        {
            if (m_uiSubPhase == SUBPHASE_TALK)
            {
                if (m_uiTimer <= uiDiff)
                {
                    ...
                    m_creature->GetMotionMaster()->Clear();
                    m_uiLastWP = urand(0, 3);
                    m_creature->GetMotionMaster()->MovePoint(POINT_ID_PHASE_2_WP, WPs[m_uiLastWP].x, WPs[m_uiLastWP].y, AIR_Z);
                    m_uiTimer = 15000;
                }
                else
                    m_uiTimer -= uiDiff;
						...
						if (!lPlayers.isEmpty())
							for (Map::PlayerList::const_iterator itr = lPlayers.begin(); itr != lPlayers.end(); ++itr)
								if (Player* pPlayer = itr->getSource())
                                {
                                    ...
                                    {
                                        pSkytalon->SetUInt32Value(UNIT_FIELD_BYTES_1, 50331648);
...
struct MANGOS_DLL_DECL npc_nexus_lordAI : public ScriptedAI
{
	...
	void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (pMap)
        {
			if (Vehicle* pDisk = pMap->GetVehicle(m_creature->GetVehicleGUID()))
            {
...
struct MANGOS_DLL_DECL npc_scion_of_eternityAI : public ScriptedAI
{
    ...
    void InitMovement()
    {
        ...
        Vehicle* pDisk = pMap->GetVehicle(m_creature->GetVehicleGUID());
        if (!pDisk)
            return;
    ...
    void DoNextMovement()
    {
        ...
        uint8 uiTmp = urand(0,2); // 0 nothing, 1 incerase, 2 decerase
        if (uiTmp == 1 && canIncerase)
            m_fDistance += 0.5f;
        else if (uiTmp == 2 && canDecerase)
            m_fDistance -= 0.5f; 
...
struct MANGOS_DLL_DECL npc_alexstraszaAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        m_creature->SetSplineFlags(SPLINEFLAG_FLYING);
        if (m_pInstance)
		    if (Creature* pMalygos = (Creature*)Unit::GetUnit(*m_creature, m_pInstance->GetData64(NPC_MALYGOS)))
...
{
    if (Unit* pMalygos = Unit::GetUnit(*pGo, pGo->GetInstanceData()->GetData64(NPC_MALYGOS)))
    {
        ...
        pMalygos->GetMotionMaster()->MovePoint(POINT_ID_LAND, CENTER_X, CENTER_Y, FLOOR_Z+7.0f);
    }
    return false;
}
