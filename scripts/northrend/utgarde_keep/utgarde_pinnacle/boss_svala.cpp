...
SDName: Boss_Svala
SD%Complete: 95%
SDComment: Нужна реализация эвента огня. Немного коряво работает паралич, его лучи направлены не на цель, а на кастера.
SDComment: Меч летит иногда криво, причина: телепорт на большом  расстоянии от алтаря.
...
enum
{
    ...
    NPC_FLAME_CAGE              = 27273,

    SPELL_ARTHAS_VISUAL         = 54134,
    SPELL_ARTHAS_DESPAWN        = 70673,
...
struct MANGOS_DLL_DECL npc_ritual_channelerAI : public Scripted_NoMovementAI
{
    ...
    void UpdateAI(const uint32 uiDiff)
    {
         if(!Stun)
         {
            if(ActionTimer < uiDiff)
            {
               m_creature->InterruptNonMeleeSpells(false);
               if(pTarget)
                 DoCast(pTarget, SPELL_PARALYZE);
               Stun = true;
            } else ActionTimer -= uiDiff;
         }
         
         if(Stun && pTarget && !pTarget->HasAura(SPELL_PARALYZE))
         {
            m_creature->InterruptNonMeleeSpells(false);
            DoCast(pTarget, SPELL_PARALYZE);
         }
...
struct MANGOS_DLL_DECL boss_svalaAI : public ScriptedAI
{
    ...
    void MoveInLineOfSight(Unit* pWho)
    {
        if (!m_bIsIntroDone)
        {
            if (m_pInstance && m_pInstance->GetData(TYPE_SVALA) == IN_PROGRESS)
            {
                ...
                m_creature->SummonCreature(NPC_ARTHAS_IMAGE, 296.151f, -356.771f, 90.948f, 1.568f, TEMPSUMMON_TIMED_DESPAWN, 70000);
            }
    ...
    void DoMoveToPosition()
    {
        ...
        m_creature->GetMap()->CreatureRelocation(m_creature, fX, fY, fZ + 7.5f, m_creature->GetOrientation());
    }

    void SpellHitTarget(Unit *target, const SpellEntry *spell)
    {
        if(spell->Id == SPELL_CALL_FLAMES)
          if(target->GetTypeId() != TYPEID_PLAYER && target->GetEntry() == NPC_FLAME_CAGE)
            StartFlameEvent(target);
    ...
    void StartFlameEvent(Unit* flame)
    {
        //Fix me. Need Find SpellId flame damage aura.
    }
    ...
    void SpellHit(Unit* pCaster, const SpellEntry* pSpell)
    {
        ...
        if(pSpell->Id == SPELL_ARTHAS_DESPAWN)
           if(pArthas)
              pArthas->SetDisplayId(11686);
    ...
    void UpdateAI(const uint32 uiDiff)
    {
                ...
                if (m_uiIntroTimer < uiDiff)
                {
                    ...
                    switch(m_uiIntroCount)
                    {
                        ...
                        case 4:
                            ...
                            break;
                        case 5:
                            pArthas->CastSpell(pArthas, SPELL_ARTHAS_DESPAWN, false);
          ...
          else
        {
          if (m_uiPhase == NORMAL)
          {
            ...
            } else m_uiSinsterStrikeTimer -= uiDiff;
            ...
            { 
                if (Unit* pSacrificeTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0))
                {
                    ...
                    SpellEntry* pTempSpell = (SpellEntry*)GetSpellStore()->LookupEntry(SPELL_RITUAL_OF_SWORD);
                    if (pTempSpell)
                    {
                       ...
                       m_creature->CastSpell(pSacrificeTarget, pTempSpell, true); //spell ports caster but not target
                    }
                    ...
                    for(uint8 i = 0; i < 3; ++i)
                       if(Creature* pSummon = m_creature->SummonCreature(NPC_RITUAL_CHANNELER, RitualChannelerPos[i].x,RitualChannelerPos[i].y,RitualChannelerPos[i].z, 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 27000))
                           ((npc_ritual_channelerAI*)pSummon->AI())->DoAction(pSacrificeTarget);
                }
            } else m_uiRitualOfSwordTimer -= uiDiff;
            ...
            if (m_uiSacrificeTimer <= uiDiff)
            {
               switch(m_uiRitualStage)
               {
                  case 1:
                     ...
                     m_uiSacrificeTimer = 250;
                     m_uiRitualStage++;
                     break;
                  case 2:
                     m_creature->NearTeleportTo(296.632f, -346.075f, 107.63f, 0);
                  ...
                  case 4:
                     ...
                     m_creature->InterruptNonMeleeSpells(false);
                     m_creature->GetMotionMaster()->MovePoint(0, 296.632f, -346.075f, 93.63f);
                     m_uiSacrificeTimer = 5000;
                     m_uiRitualStage++;
                     break;
                  case 5:
                     m_uiPhase = NORMAL;
